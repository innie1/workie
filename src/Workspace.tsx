...
function Docs({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  useEffect(() => { const items = listDocuments(); setDocs(items); if (items.length) { setCurrent(items[0]); setTitle(items[0].title); setBody(items[0].body) } }, [])
  ...
  const remove = () => { if (!current) return; deleteDocument(current.id); const left = listDocuments(); setDocs(left); if (left.length) open(left[0]); else { setCurrent(null); setTitle(''); setBody('') } onNotice('Document deleted.') }
  return <div className="editor-shell"><div className="editor-toolbar"><div className="doc-top"><button className="new-doc" onClick={newDoc}><Plus size={16}/> New document</button>{docs.length>0&&<select value={current?.id || ''} onChange={e=>{const d=docs.find(x=>x.id===e.target.value);if(d)open(d)}}>{docs.map(d=><option key={d.id} value={d.id}>{d.title}</option>)}</select>}</div><div className="editor-actions">{current&&<><button onClick={save}><Save size={16}/> Save</button><button onClick={()=>download(`${title || 'document'}.txt`, `${title}\n\n${body}`, 'text/plain')}><Download size={16}/> Export</button><button onClick={remove}><Trash2 size={16}/></button></>}</div></div><div className="doc-layout"><aside className="doc-sidebar"><div className="doc-sidebar-head"><strong>Documents</strong><button onClick={newDoc}><Plus size={16}/></button></div>{docs.map(d=><button className={`doc-list-item ${d.id===current?.id?'selected':''}`} key={d.id} onClick={()=>open(d)}><FileText size={15}/><span>{d.title}</span></button>)}</aside><div className="doc-paper">{current?<div className="doc-page"><div className="doc-tools"><button onClick={()=>setBody(v=>v+'\n\n**Bold**')}>B</button><button onClick={()=>setBody(v=>v+'\n\n*Italic*')}>I</button><button onClick={()=>setBody(v=>v+'\n\n# Heading')}>H1</button><button onClick={()=>setBody(v=>v+'\n\n- List item')}>• List</button></div><input className="doc-title" value={title} onChange={e=>setTitle(e.target.value)} /><textarea value={body} onChange={e=>setBody(e.target.value)} placeholder="Start writing here..." /></div>:<div className="empty"><FileText size={28}/><strong>No documents yet</strong><p>Create your first document to begin writing.</p><button className="primary" onClick={newDoc}><Plus size={16}/> New document</button></div>}</div></div></div>
}

function Sheets({ onNotice }: { onNotice: (message: string) => void }) {
  const [rows,setRows]=useState<Row[][]>(()=>{try{return JSON.parse(readProjects().sheet?.rows||'null')||[['','','',''],['','','',''],['','','',''],['','','','']]}catch{return [['','','',''],['','','',''],['','','',''],['','','','']]}})
  const update=(r:number,c:number,v:string)=>setRows(x=>x.map((row,ri)=>ri===r?row.map((cell,ci)=>ci===c?v:cell):row))
  const add=()=>setRows(x=>[...x, ['', '', '', '']])
  const total=useMemo(()=>rows.reduce((n,r)=>n+(Number(r[1])||0)*(Number(r[2])||0),0),[rows])
...
function Slides({ onNotice }: { onNotice: (message: string) => void }) {
  const [slides,setSlides]=useState<any[]>(()=>readProjects().slides?.slides||[]),[active,setActive]=useState(0)
  const current=slides[active]
  const update=(key:'title'|'body',value:string)=>setSlides(s=>s.map((x,i)=>i===active?{...x,[key]:value}:x))
  const add=()=>{setSlides(s=>[...s,{title:'',body:''}]);setActive(slides.length)}
  const save=()=>{writeProjects({...readProjects(),slides:{slides}});onNotice('Presentation saved.')}
  return <div className="slides-shell"><div className="editor-toolbar"><strong>Slides</strong><div className="editor-actions"><button onClick={add}><Plus size={16}/> Slide</button>{slides.length>0&&<><button onClick={save}><Save size={16}/> Save</button><button onClick={()=>download('workie-presentation.txt',slides.map((s,i)=>`SLIDE ${i+1}\n${s.title}\n${s.body}`).join('\n\n'),'text/plain')}><Download size={16}/> Export</button></>}</div></div><div className="slides-layout">{slides.length>0?<><aside className="slide-list">{slides.map((s,i)=><button key={i} className={i===active?'selected':''} onClick={()=>setActive(i)}><span>{i+1}</span><b>{s.title||'Untitled slide'}</b></button>)}</aside><div className="slide-canvas"><input className="slide-title" value={current.title} placeholder="Slide title" onChange={e=>update('title',e.target.value)} /><textarea className="slide-body" value={current.body} placeholder="Add your content..." onChange={e=>update('body',e.target.value)} /><button className="primary" onClick={()=>onNotice('Presentation preview ready.') }><Play size={16}/> Present</button></div></>:<div className="empty"><LayoutPlaceholder/><strong>No slides yet</strong><p>Create a blank slide to begin your presentation.</p><button className="primary" onClick={add}><Plus size={16}/> New slide</button></div>}</div></div>
}
function LayoutPlaceholder(){return <Sparkles size={28}/>}

function Design({ onNotice }: { onNotice: (message: string) => void }) {
  const [title,setTitle]=useState(''),[subtitle,setSubtitle]=useState(''),[size,setSize]=useState('1080 × 1080')
  const save=()=>{writeProjects({...readProjects(),design:{title,subtitle,size}});onNotice('Design saved.')}
  return <div className="design-shell"><div className="editor-toolbar"><div><strong>Design Studio</strong><small> {size}</small></div><div className="editor-actions"><select value={size} onChange={e=>setSize(e.target.value)}><option>1080 × 1080</option><option>1920 × 1080</option><option>1080 × 1920</option><option>1200 × 628</option></select><button onClick={save}><Save size={16}/> Save</button><button onClick={()=>download('workie-design.txt',`${title}\n${subtitle}\nSize: ${size}`,'text/plain')}><Download size={16}/> Export</button></div></div><div className="design-stage"><div className="design-card"><input value={title} placeholder="Title" onChange={e=>setTitle(e.target.value)} /><textarea value={subtitle} placeholder="Add a description" onChange={e=>setSubtitle(e.target.value)} /><span>WORKIE DESIGN</span></div></div></div>
}

function Motion({ onNotice }: { onNotice: (message: string) => void }) {
  const [playing,setPlaying]=useState(false),[duration,setDuration]=useState(10),[text,setText]=useState(''),[x,setX]=useState(20),[frames,setFrames]=useState<number[]>([])
...
function MediaWorkspace({kind,onNotice}:{kind:string;onNotice:(m:string)=>void}) {
  ...
  const [name,setName]=useState(''),[prompt,setPrompt]=useState(''),[items,setItems]=useState<string[]>([])

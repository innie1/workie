...
import { askWorkieAI, getGemmaStatus, loadLocalGemma } from './workie-ai'
...
function App(){
 const[active,setActive]=useState('Home'),[query,setQuery]=useState(''),[create,setCreate]=useState(false),[ai,setAi]=useState(false),[prompt,setPrompt]=useState(''),[answer,setAnswer]=useState(''),[busy,setBusy]=useState(false),[notice,setNotice]=useState(''),[gemma,setGemma]=useState(getGemmaStatus())
 ...
 const ask=async(text=prompt)=>{
  ...
  try{
   ...
   if(action){
    ...
    setAnswer((await askWorkieAI({prompt:text})).text)
   }
  }catch(e){setAnswer(e instanceof Error?e.message:'Gemma could not respond.')}finally{setBusy(false)}
 }
 ...
 return <div className="app-shell">
  ...
  <main className="main"><header className="topbar"><button className="mobile-menu"><Menu size={20}/></button><div className="search"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search files, tools, templates..."/><kbd>Ctrl K</kbd></div><div className="top-actions"><div className="avatar">W</div></div></header>
   <div className="content">{active==='Home'?<>
    ...
    <section className="lower-grid"><div className="panel templates"><div className="panel-head"><h2>Templates</h2><button>View all</button></div><div className="template-grid">{['Project Proposal','Marketing Plan','Business Report','Pitch Deck','Invoice','Social Post'].map((x,i)=><button key={x} onClick={()=>flash(`${x} selected.`)}><div className={`template-preview p${i}`}><div/><span/><span/></div><strong>{x}</strong></button>)}</div></div><div className="right-stack"><div className="panel ai-card"><div className="ai-head"><span className="ai-symbol"><Sparkles size={20}/></span><div><h2>Workie AI</h2><small>{gemma.ready?'Gemma 4 is installed locally':'Gemma 4 • ready to install'}</small></div><span className="model">{gemma.ready?'LOCAL':'E2B'}</span></div><div className="ai-input"><textarea value={prompt} onChange={e=>setPrompt(e.target.value)} placeholder="Ask Workie to create, edit, analyze or organize..."/><button onClick={()=>ask()} disabled={busy}><ArrowRight size={18}/></button></div>{answer&&<div className="empty"><strong>{busy?'Thinking…':'Workie AI'}</strong><p>{answer}</p></div>}<div className="chips"><button onClick={()=>ask('Create a presentation')}>Create a presentation</button><button onClick={()=>ask('Create a motion graphic')}>Create motion graphic</button></div>{!gemma.ready&&<button className="gemma-install" onClick={installGemma}><Sparkles size={15}/> Install Gemma 4 locally (~3 GB)</button>}</div><div className="panel motion-card"><div className="panel-head"><h2>Motion Studio</h2><button onClick={()=>go('Motion Studio')}>New Project</button></div><button className="motion-preview" onClick={()=>go('Motion Studio')}><span className="play">▶</span></button><strong>Brand Intro Animation</strong><small>Motion graphics workspace</small></div></div></section>
